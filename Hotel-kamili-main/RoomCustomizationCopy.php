...
<body>
    ...
    <tr>
        ...
        <div id="cake_options" style="display: none;">
            ...
            <label for="cake_message">Custom wording on Cake (Max 8 words)</label>
<input type="text" id="cake_message" name="cake_message" placeholder="Enter cake message" oninput="checkWordLimit()" required>
<p id="word_limit_warning" style="color: red; display: none;">Word limit exceeded. Maximum 8 words allowed.</p>
...
<br>
    ...
    const inputField = document.getElementById('cake_message');
    const warningMessage = document.getElementById('word_limit_warning');
    const words = inputField.value.trim().split(/\s+/); // Split the input by spaces

    if (words.length > 8) {
        warningMessage.style.display = 'block'; // Show warning
        inputField.value = words.slice(0, 8).join(' '); // Limit the input to 8 words
    } else {
